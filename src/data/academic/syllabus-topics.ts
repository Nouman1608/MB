  ...
  },
{
    ...
    verifiedDate: '2026-09-08',
    notes: 'Fetched the official 2-page IB subject brief PDF via a rendered browser session (direct HTTP fetch was intercepted by an IBO Cloudflare bot-check). Section II, "Curriculum model overview" (page 2), is the only place the brief lists syllabus content, and it lists two flat, unnumbered groups: "Core" (3 approaches to understanding behaviour, compulsory for all students) and "Options" (4 areas, of which students study one), plus a research-methodology strand and an internal-assessment experimental study component that is an assessment task rather than syllabus content and so is excluded from the topics below rather than modeled as a content topic. No decimal or numbered sub-content is given anywhere in the brief, so subtopics are [] throughout. This record matches this database\'s companion syllabuses.ts entry, which states this first-assessment-2019 course is "the one current DP cohorts are actually examined under today" (verified 2026-09-08) alongside a newer first-assessment-2027 course (first teaching August 2025) already tracked separately in this database as a future syllabus generation. Flag for future review: independent web searches performed during this research pass (not the IB subject brief itself) suggested May 2026 may have been the final standard examination session for the 2019 generation in some regions/contexts; this was not corroborated against an authoritative IB source and is not reflected in the status below, which follows the existing, more directly IB-sourced syllabuses.ts record. A future pass should re-verify current examination-session status directly against IB\'s own curriculum-updates page before this is relied upon.',
    topics: [
