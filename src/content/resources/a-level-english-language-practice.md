...
**1. Broadening** — a word's meaning becomes **more general**, e.g. "holiday" from "holy day" to any period of leave [1]. **Narrowing** — a word's meaning becomes **more specific**, e.g. "meat" from any food to animal flesh [1]. **Amelioration** — a word acquires **more positive connotations**, e.g. "nice", once meaning ignorant [1]. **Pejoration** — a word acquires **more negative connotations**, e.g. "silly", once meaning blessed or innocent [1].

**2. Prescriptivism** holds that there is a **correct form of the language which should be taught and enforced**, and that deviation is error [1]. **Descriptivism** holds that the linguist's task is to **record and analyse how the language is actually used**, without judging it [1]. The descriptivist regards change as **natural and inevitable**, where the prescriptivist regards it as decay [1].
...
**6.** An over-generalisation occurs when a child **applies a regular grammatical rule to an irregular form** [1]. For example, saying **"goed" for "went" or "sheeps" for "sheep"** [1].
...
- Confusing broadening with amelioration.
- Giving stages of acquisition without approximate ages or features.
- Describing a theory without a criticism when one is asked for.
