...
[Periodic Table: Groups and Trends study guide](/resources/periodic-table-groups-and-trends/).

## How the table is arranged

- **Groups** (columns) = number of **outer-shell electrons**. Same group → similar chemical properties.
- **Periods** (rows) = number of **occupied shells**.
- Metals on the left, non-metals on the right, with a staircase boundary.

Arranged in order of **proton (atomic) number**.
...
All are soft, low-density metals stored under oil; all form 1+ ions and alkaline hydroxides.
...
Reactivity falls because the outer shell is further from the nucleus, so an electron is **gained** less easily.

**Displacement:** a more reactive halogen displaces a less reactive halide from solution.
`Cl₂ + 2KBr → 2KCl + Br₂` (solution turns orange)
...
Group VIII/0 — full outer shell, therefore **unreactive** (monatomic). Uses: helium in balloons (low density, non-flammable), argon in lamps and welding (inert atmosphere), neon in lighting.
