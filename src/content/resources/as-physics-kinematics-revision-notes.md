...
Assessed on **Paper 1** (multiple choice, 1 h 15, 40 marks) and **Paper 2** (structured, 1 h 15, 60 marks), both testing AO1 and AO2 on AS content.
...
Vectors combine by the parallelogram rule or by resolving into perpendicular components:
...
For a curve, take the gradient of the **tangent** at the point.

Note "displacement", not "distance": area below the axis on a velocity–time graph is negative displacement.

## Projectile motion — the whole method

Treat horizontal and vertical **independently**. They share only the time.
...
For a projectile launched at angle θ with speed u: `u_x = u cos θ`, `u_y = u sin θ`.

## Air resistance

Without it, the trajectory is a symmetrical parabola. **With** air resistance the path becomes asymmetric: reduced range and maximum height, a steeper descent than ascent, and the impact speed is less than the launch speed.
