...
**(b)** Write pseudocode to declare this array and a loop that calculates the total goals scored by the team, indexing from 0. **[4]**
...
**7.** Write pseudocode for nested loops that visit every element of a 2-D array called grid, with dimensions grid[5][4] indexed from 0, and set every element to zero. **[4]**

**8.** AQA's specification describes arrays as "(or equivalent)". Explain what this phrase means and why it is included. **[2]**

**9.** AQA sets no pseudocode convention of its own at AS or A-level. Explain why a programmer must still check a language or convention's index origin before writing a loop that must visit every element exactly once, and state what goes wrong if a 0-based array is looped as though it were 1-based (or vice versa). **[3]**
...
**(b)**
```
goals = array of 11 integers, indexed 0 to 10
total = 0
FOR i = 0 TO 10
    total = total + goals[i]
ENDFOR
...
**9.** AQA sets no pseudocode command set of its own at AS or A-level, so a programmer cannot assume an index origin — they must check the specific language or convention their course uses [1]. This matters because a loop must run across the array's actual index range (0 to size − 1 if 0-based, or 1 to size if 1-based) to visit every element exactly once [1]; looping a 0-based array as though it were 1-based either misses the first element or attempts to access an element one past the end that does not exist, causing an error [1].
...
- Assuming a fixed index origin instead of checking the course's language or convention, causing an off-by-one error.
