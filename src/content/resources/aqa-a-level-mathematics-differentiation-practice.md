...
**(b)** By the chain rule for parametric differentiation, dy/dx = (dy/dt) ÷ (dx/dt) = 6t²/2t = 3t (for t ≠ 0). At t = 2: dy/dx = 3(2) = 6. Note that the division cancels a factor of t rather than t², since 6t²/2t = 3t, not 3t² — a frequent slip is cancelling incorrectly. Substituting t = 2 into dy/dt and dx/dt first and then dividing (24 ÷ 4 = 6) is not a slip: it is equally valid, because dy/dx = (dy/dt) ÷ (dx/dt) holds at every value of t where dx/dt ≠ 0, so it always gives the same value as substituting into 3t. Simplifying first is still the better habit here, because part (b) asks for dy/dx in terms of t before the evaluation. **[3]**
