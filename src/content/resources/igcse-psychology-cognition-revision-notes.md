---
title: "IGCSE Psychology: Cognition and Behaviour — Revision Notes"
...
[Cognition and Behaviour study guide](/resources/igcse-oxfordaqa-psychology-cognition-and-behaviour/).

## Memory

**Encoding → storage → retrieval.**

**Multi-store model:** sensory register (very brief, large capacity) → **attention** → short-term memory (5–9 items, 18–30 seconds) → **rehearsal** → long-term memory (potentially unlimited).
...
|---|---|---|
| **Laboratory experiment** | High control, cause established | Artificial, low ecological validity |
| **Field experiment** | More realistic | Less control over variables |
| **Observation** | Natural behaviour | No control, observer bias |
| **Questionnaire** | Large samples, quick | **Social desirability bias**, low validity |
| **Interview** | Detailed, flexible | Time-consuming, interviewer effect |
| **Case study** | Rich detail on rare cases | Not generalisable |

- **Independent variable** — what is changed. **Dependent variable** — what is measured. **Extraneous variables** must be controlled.
- **Hypothesis** — a testable prediction. The **null hypothesis** predicts no effect.
...
1. Give the capacity and duration of short-term memory.
2. What is retrieval failure, and what does it explain that decay cannot?
3. What did Bartlett's study demonstrate?
4. Which theory explains illusions well, and what is its weakness?
5. Distinguish the independent from the dependent variable.

**Answers:** 1. Five to nine items, held for roughly 18 to 30 seconds. 2. Forgetting caused by the absence of the cue needed to access a stored memory; it explains why a memory can be recovered later with the right prompt, which decay cannot. 3. That memory is reconstructive — recall is shaped by cultural schemas, so details are omitted, shortened or altered to fit expectations. 4. Gregory's constructivist theory; its weakness is that it struggles to explain how perception is normally so fast and accurate. 5. The independent variable is what the researcher changes; the dependent variable is what is measured.
