---
title: "Edexcel IGCSE History: Depth Studies — Revision Notes"
resourceType: "revision-notes"
subject: "world-history"
level: ["igcse"]
topic: "Paper 1 – Depth Studies (choose at least 2 of 8)"
boards: ["edexcel"]
qualifications: ["igcse"]
syllabusCodes: ["4HI1"]
syllabusSeries: "Issue 5, February 2026"
order: 1
syllabusTopics:
  - qualification: "igcse"
    topic: "paper-1-depth-studies-4hi1"
description: "Condensed recall notes on the interpretation-evaluation skill Paper 1 tests, causation, and the depth-study topics for Edexcel International GCSE History 4HI1."
author: "marlbridge-academic-team"
publishedDate: 2026-08-22
featured: false
---

Condensed for the final weeks. For the full explanation, use the
[Depth Studies study guide](/resources/igcse-edexcel-world-history-paper-1-depth-studies/).

## Working with the extract — the skill Paper 1 actually tests

Paper 1 carries **no source-handling marks at all** (0% AO3) -- unlike Paper 2, it is not a
source-evaluation paper, and "how far do the sources agree" questions belong to Paper 2, not here
(see the [Paper 2 revision notes](/resources/edexcel-igcse-world-history-paper-2-revision-notes/)
for that skill). Each depth-study question opens with a short printed extract, but that extract is a
**secondary historian's interpretation** (attributed to a named author and a publication date), not
a primary source -- and it is assessed under the interpretations objective (AO4), a different skill
with a different mark scheme from source evaluation.

Part (a) of every question (6 marks) asks: *"What impression does the author give about [topic]?
You must use Extract [X] to explain your answer."* This tests whether you can accurately read the
extract's language and tone to state the author's own viewpoint -- not whether the extract is
reliable, biased, or useful for a stated enquiry. Do not apply a source-reliability framework
(origin/purpose/usefulness) here: the question wants what the author's impression *is*, drawn from
specific words and phrasing in the extract, not a judgement about whether to trust it.

**A biased author is not a wrong answer to avoid.** The extract's author may write with a clear
slant -- that slant is itself part of the "impression" you are asked to identify and explain, not a
reason to distrust or dismiss the extract.

## Causation

Examiners reward **weighing**, not listing.

The structure that scores:

1. Explain how each factor contributed — not just that it did.
2. **Link factors** where they reinforce each other.
3. Judge which mattered most, and **state the criterion** for the judgement.

**Distinguish the trigger from the underlying causes.** The immediate event starts things, but only because the conditions made escalation possible. "The spark is not the cause" applies across the depth studies.

## Recurring themes across the depth studies

**Treaties and their consequences** — the Treaty of Versailles was **harsh enough to embitter Germany but not harsh enough to keep her weak**. That single assessment answers most Versailles evaluation questions.

**Why international organisations fail** — the League of Nations failed for **structural** reasons, not one incidental one: the USA never joined, it had no army, decisions required unanimity, key powers were excluded, sanctions were weak, and Britain and France pursued their own interests. Naming several structural causes beats naming one.

**Dictatorship and control** — the mechanisms recur across topics: propaganda, censorship, a secret police, youth organisations, control of education, elimination of opposition, and economic policy used to buy consent. Being able to apply that framework to any regime is more useful than memorising one.

**Appeasement** — argue both sides. *For:* Britain was militarily unprepared, public opinion feared war, Versailles was widely seen as unjust, and the USSR seemed the greater threat. *Against:* it emboldened Hitler, sacrificed Czechoslovakia, and let Germany grow stronger relatively.

**Cold War origins** — ideological opposition, wartime suspicion, the atomic bomb, and disagreements at Yalta and Potsdam over Poland and Germany.

## The eight depth-study options

| Option | Period | Focus |
|---|---|---|
| 1. French Revolution | c1780–99 | Political upheaval and revolutionary change in France |
| 2. Unification of Italy | 1848–70 | Nation-building and a unified state |
| 3. Germany: Dictatorship | 1918–45 | The rise and consolidation of Nazi rule |
| 4. India: Nationalist Challenge | 1919–47 | Colonial rule and the movement toward independence |
| 5. USSR: Dictatorship and Conflict | 1924–53 | Stalinist rule and its consequences |
| 6. Superpower Relations | 1943–72 | Cold War tensions between the USA and USSR |
| 7. US Civil Rights | 1945–74 | The struggle for racial equality in the United States |
| 8. South Africa: Apartheid | 1948–94 | The rise and fall of institutionalised racial segregation |

**Confirm which two (or more) options your centre actually teaches** before revising in depth — studying the wrong option, even briefly, wastes time that could deepen knowledge of the period that will genuinely appear in the exam.

## Structuring revision by option

Build **separate notes for each studied option**, rather than one combined set: a chronological outline of major events, the key individuals and their roles, the causes of major developments, and the consequences or significance of those developments.

**Keep periods separate.** Candidates lose marks by unintentionally applying facts from one Depth Study to a question about a different one — particularly where two studied options share superficial similarities, such as both involving a shift toward authoritarian rule.

## Writing the answers

- Use **precise** evidence: names, dates, figures, treaties. "Several wars" is weak; "three wars between 1864 and 1871" is strong.
- Every paragraph should make a point, support it, and **link back to the question**.
- Conclusions must **judge**, not summarise — and say what the judgement rests on.
- Answer the question set, not the one you revised for.

## Exam traps

- Applying a source-reliability framework (origin/purpose/usefulness) to the extract, when part (a)
  wants the author's impression, not a judgement about whether to trust the extract.
- Describing the extract's content instead of stating the impression it gives.
- Listing causes without explaining or weighing them.
- Narrating events rather than arguing a case.
- Vague evidence with no dates or names.
- A conclusion that restates rather than judges.
- Confusing facts between two studied options that share surface similarities, such as two authoritarian-rule topics.
- Revising the wrong Depth Study option without confirming which two the centre actually teaches.

## Self-test

1. What does Paper 1's opening extract actually assess -- source reliability or something else?
2. What does part (a) of every depth-study question ask for, and how many marks is it worth?
3. What structure earns marks in a causation question?
4. Summarise the central problem with the Treaty of Versailles.
5. Give four structural reasons the League of Nations failed.
6. Name the eight Depth Study options and the period each covers.
7. Why should revision notes be built separately for each studied option rather than combined into one set?

**Answers:** 1. Something else -- the extract is a secondary historian's interpretation assessed under the interpretations objective (AO4), not a primary source assessed for reliability; Paper 1 carries no source-handling (AO3) marks at all. 2. "What impression does the author give about [topic]? You must use Extract [X] to explain your answer" -- worth 6 marks. 3. Explain how each factor contributed, link factors together, then judge which mattered most and state the criterion for that judgement. 4. It was harsh enough to embitter Germany but not harsh enough to prevent her recovering strength. 5. The USA never joined; it had no army; decisions required unanimity; key powers were excluded; sanctions were ineffective; Britain and France pursued self-interest — any four. 6. French Revolution (c1780-99), Unification of Italy (1848-70), Germany: Dictatorship (1918-45), India: Nationalist Challenge (1919-47), USSR: Dictatorship and Conflict (1924-53), Superpower Relations (1943-72), US Civil Rights (1945-74), South Africa: Apartheid (1948-94). 7. Because candidates sometimes unintentionally apply facts from one Depth Study to a question about a different one, especially where two studied options share superficial similarities such as both involving authoritarian rule; separate notes keep the periods from bleeding into one another.
