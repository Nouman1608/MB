---
title: "O Level World History: Depth Studies — Revision Notes"
resourceType: "revision-notes"
subject: "world-history"
level: ["o-levels"]
topic: "Depth Studies"
boards: ["cambridge"]
qualifications: ["o-level"]
syllabusCodes: ["2147"]
syllabusSeries: "2024-2026"
order: 3
syllabusTopics:
  - qualification: "o-level"
    topic: "depth-studies-2147"
description: "Condensed recall notes on the five Depth Studies, how Paper 1 Section B examines them, and the source-evaluation technique Paper 2 tests on the Core Content prescribed topic, for Cambridge O Level World History (2147)."
author: "marlbridge-academic-team"
publishedDate: 2026-09-06
featured: false
---

Condensed for the final weeks. For the full explanation, use the
[Depth Studies study guide](/resources/o-level-world-history-depth-studies/).

## Where this fits

All candidates study Core Content (Option A 19th century, or Option B 20th century) **plus at least one Depth Study**, assessed in **Paper 1, Section B**: two structured essay questions in three parts, (a)–(c), on each Depth Study, answer one; knowledge (AO1) + explanation (AO2). Depth Study preparation = detailed knowledge + explanation, not source work. **Source evaluation (AO3) is Paper 2 (Document Questions)**, set on a Core Content prescribed topic, never on a Depth Study — the source notes below are that transferable Paper 2 skill.

## The five Depth Studies

- **A. First World War, 1914–18** — causes, course, consequences; Western Front stalemate.
- **B. Germany, 1918–45** — Weimar's struggles, Nazi Party's rise, life in Nazi Germany.
- **C. Russia, 1905–41** — causes/impact of 1905 and 1917 revolutions; Soviet power consolidation under Lenin and Stalin.
- **D. United States, 1919–41** — 1920s social/economic change, Depression's causes/impact, the New Deal.
- **E. Second World War, 1939–c.1945** — causes, course, consequences across Europe AND Asia-Pacific theatres.

## Source evaluation — the Paper 2 skill (Core Content prescribed topic)

Comment on a source's **content**, its **origin** (who, when, purpose), and its **value/limitations as evidence** for a specific question — never just describe what it shows. Practise across source types (photographs, posters, cartoons, official documents, personal accounts) since you can't know in advance which type an exam will use.

## Depth-study-specific approach

- **A (WWI):** explain WHY the Western Front became a stalemate (trench warfare + machine guns + artillery + barbed wire making rapid advance too costly), not just narrate battles.
- **B (Germany):** trace a clear cause-and-effect line from Weimar's specific weaknesses (economic crises, political instability, Versailles' legacy) to conditions allowing Nazi power — don't treat Nazi Germany as a disconnected starting point.
- **C (Russia):** distinguish 1905 (failed to remove the Tsar) from 1917 (succeeded) as **distinct phases** with different causes/results, not one continuous revolution; then trace consolidation under Lenin, then Stalin.
- **D (USA):** explain the Wall Street Crash/Depression as a **connected causal sequence**, not a sudden unexplained event; evaluate specific New Deal measures, not just "government intervention" in general.
- **E (WWII):** prepare European and Asia-Pacific theatres as **related but distinct strands**, able to draw connections between them (e.g. how European events affected Japan's strategic calculations).

## Cross-referencing sources — also a Paper 2 skill

Paper 2 often presents two+ sources on the same event and asks for comparison: note where they **agree**, where they **differ**, and suggest **why** (different authors/audiences/purposes often explain apparently contradictory sources on the same event).


## Worked example: evaluating a source's origin and purpose

A practice source task (the Paper 2 skill, applied here to Depth Study B material — Paper 2 itself is set on a Core Content prescribed topic) presents a 1936 German government poster celebrating the achievements of the Nazi regime, and asks candidates to assess its value as evidence for how ordinary Germans viewed the regime at the time. A weak answer describes what the poster shows (for example, images suggesting prosperity and national unity). A stronger answer evaluates its origin and purpose directly: because the poster was produced BY the German government FOR public display, its purpose was almost certainly propaganda -- to shape public opinion rather than to record it -- which limits its value as direct evidence of how ordinary Germans actually felt, even though it may still be valuable evidence of what the regime wanted people to believe, or of the regime's own priorities and self-presentation. This distinction, between a source's value as evidence of the event it depicts versus its value as evidence of the purpose behind its creation, is exactly the kind of judgement Paper 2 rewards over description alone.

## Worked example: comparing two sources that appear to disagree

Two sources describe the same 1917 event in Russia: one, a Bolshevik Party account, presents it as a spontaneous popular uprising; another, a foreign diplomat's private report, describes it as a carefully organised seizure of power by a small group. Rather than treating this disagreement as a problem to resolve by picking the "correct" source, a strong answer explains why the two sources might genuinely differ given their origin and purpose: the Bolshevik account has an interest in presenting the revolution as broadly popular to legitimise the new government, while the diplomat's private report, not intended for public consumption, may have less incentive to present a particular political narrative, though it also reflects only one outside observer's limited access to what was actually happening. Explaining the disagreement through each source's origin and purpose, rather than simply declaring one source "more reliable," is what genuine source cross-referencing looks like in practice.

## Exam traps

- Narrating events chronologically without ever answering the actual question each part, (a), (b) or (c), asks.
- Describing a source's content without evaluating its origin, purpose or reliability.
- Treating 1905 and 1917 in Russia as one continuous event.
- Explaining Hitler's rise with no reference to Weimar's specific preceding weaknesses.
- Discussing WWII as a single unified cause/course rather than distinct (connected) theatres.
- Comparing two sources without suggesting *why* they might differ (author, audience, purpose).

## Self-test

1. Name the five Depth Studies.
2. What three things should a source-evaluation answer comment on?
3. Why should 1905 and 1917 in Russia be treated as distinct phases?
4. What should a strong Depth Study B answer trace, and from where?
5. Why should the WWII theatres be prepared as "related but distinct," rather than one unified narrative?
6. Give one reason two sources on the same event might genuinely differ.

**Answers:** 1. The First World War (1914–18); Germany (1918–45); Russia (1905–41); The United States (1919–41); The Second World War (1939–c.1945). 2. Content, origin (who produced it, when, for what purpose), and its value or limitations as evidence for a specific question. 3. Because 1905 failed to remove the Tsar while 1917 succeeded — they have different causes and different outcomes, so treating them as one continuous revolution misses this distinction. 4. A clear line of cause and effect from the Weimar Republic's specific weaknesses (economic crises, political instability, the legacy of Versailles) through to the conditions that allowed the Nazi Party to gain power. 5. Because although connected (e.g. events in Europe affecting Japan's strategic calculations), the two theatres have distinct causes, courses and actors that should each be known specifically, not blended into one narrative. 6. Different authors, audiences, or purposes behind each source's creation.
