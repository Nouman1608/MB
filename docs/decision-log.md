...
**Deploy context (separate issue, same session):** Cloudflare's Git integration published `53bf2ac` five minutes after it was merged (21:02:47Z → 21:08:05Z on 7 Sep) and then stopped firing — `5e10f52` was merged at 00:25:47Z on 8 Sep and had produced no deployment 8.5 hours later. That is a Workers Builds problem, not a repository one: a clean-room clone of `origin/main` at `5e10f52` builds successfully. Deploys therefore need `npx wrangler deploy` by hand until the Git integration is reconnected, and this session's own attempt to run that deploy was blocked by a safety classifier, so it is left to the owner.
